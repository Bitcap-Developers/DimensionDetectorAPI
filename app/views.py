...
def fun(request):
	...
	mask = cv2.inRange(imagetop, lower, upper)
	output = cv2.bitwise_and(imagetop, imagetop, mask = mask)
	#detect_coin = imagetop.copy()
	#gray = cv2.cvtColor(output, cv2.COLOR_HSV2GRAY)
	...
	gray = cv2.cvtColor(imagetop, cv2.COLOR_BGR2GRAY)
	...
	v2=135
	upper = np.array([h2,s2,v2])
	mask = cv2.inRange(imagefront, lower, upper)
	output = cv2.bitwise_and(imagefront, imagefront, mask = mask)
	#detect_coin = imagetop.copy()
	#gray = cv2.cvtColor(output, cv2.COLOR_HSV2GRAY)
	gray = cv2.GaussianBlur(output, (7, 7), 0)
	...
	dB = dist.euclidean((tlblX, tlblY), (trbrX, trbrY))
	# compute the size of the object
	width1 = dA * pixelsPerMetric
	length1 = dB * pixelsPerMetric
	if(width1<length1):
		height=width1
	else:
		height=length1
